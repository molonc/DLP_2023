function waitBeforeMoving(app)
    ...
    writeline(app.stage, '/'); % this will retrun the status B meaning busy N meaning no motor runnning
    ...
    if isequal(response,'N') % hoping that this doesnt mess anything up 
        ...
        waitBeforeMoving(app); %this is a recursive function which my run into some run time errors :) 
    end
end
